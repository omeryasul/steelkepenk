...
namespace Application.Features.Contents.Queries.GetBySlug
{
    public class GetContentBySlugQueryHandler : IRequestHandler<GetContentBySlugQuery, Result<ContentDetailDto>>
    {
        ...
        public GetContentBySlugQueryHandler(IApplicationDbContext context, IMapper mapper)
        {
            _context = context;
            _mapper = mapper;
        }
        ...
        public async Task<Result<ContentDetailDto>> Handle(GetContentBySlugQuery request, CancellationToken cancellationToken)
        {
            try
            {
                ...
                {
                    query = query.Where(x => x.Status == Domain.Enums.ContentStatus.Published);
                }

                var content = await query.FirstOrDefaultAsync(cancellationToken);
                ...
                if (request.IncludeNavigation)
                {
                    var previousContent = await _context.Contents
                        .Where(x => x.Id < content.Id && x.Status == Domain.Enums.ContentStatus.Published && x.CategoryId == content.CategoryId)
